...
def download_rhea_data(data_path):
    """Download necessary data from RHEA and ChEBI for mapping with p2m"""
    if os.path.exists(data_path):
        raise Exception(
            "{} already exists. Please specify a new directory to download data to.".format(
                data_path
            )
        )
    else:
    ...
    tsv_url = "ftp://ftp.expasy.org/pub/databases/rhea/tsv/"  # old url no longer maintained as of 2022-05-01
    ...
    logging.info("Generating ChEBI ID to SMILES mapping file...")
