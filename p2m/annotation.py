...
class Annotation:
    ...
    def __init__(self, ids, ids_type, paths):
        self.ids = {}
        self.ids[ids_type] = ids
        self.paths = paths
        self.rhea_ids = []
        self.rxns = []
        
    def add_ids(self, ids, ids_type):
        """
        Add IDs and ID type to those that will be queried for Rhea reactions
        """
        self.ids[ids_type] = ids
    
    #TODO: Implement addition of multiple ID types or change flow of this method.
    def map_rhea(self, external_uniprot):
        """
        Map ID's (UniProt or EC) to Rhea reaction identifiers.
        Preliminary or non-existant reaction identifiers are guessed and added to the external_rhea list.
        """
        for ids_type in self.ids.keys():
            if ids_type=='UniProt':
                rhea2uniprot = pd.read_csv(self.paths['rhea2uniprot.tsv'], sep='\t', dtype=str)
                found_uni_rheas, self.missing_uni_ids = u.get_master_rhea(rhea2uniprot, self.ids[ids_type])
                ...
                found_ec_rheas, self.missing_ec_ids = u.get_master_rhea(rhea2ec, self.ids[ids_type])
    ...
    def add_external_rhea(self):
        ...
        """
        guess_lr = [str(int(x) + 1) for x in self.rhea_rxn_ids['external']]
        guess_rl = [str(int(x) + 2) for x in self.rhea_rxn_ids['external']]
        all_externals = self.rhea_rxn_ids['external'] + guess_lr + guess_rl
        dir_externals = self.rhea_directions.isin(all_externals) #Remove guessed Rhea ID's that overlap with known Rhea IDs
        ...
        self.external_rhea_rxns = u.external_rhea(searchable_rxns) #Search for all rhea IDs not found locally
    ...
    #TODO: Ensure first drop duplicates is necessary
    def star_to_smiles(self):
        ...
        all_smiles = matched_chebis.drop_duplicates(subset='ChEBI_ID', ignore_index=True) #Is this really necessary since we drop again?
