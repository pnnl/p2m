...
class Annotation:
    ...
    def map_uniprot(self, external_uniprot):
        ...
        found_uni_rheas, self.missing_uni_ids = u.get_master_rhea(
            rhea2uniprot, self.ids
        )
    ...
    def map_rhea(self, external_uniprot):
        ...
        self.rhea_directions = pd.read_csv(
            self.paths["rhea-directions.tsv"], sep="\t", dtype=str
        )
    ...
    def add_external_rhea(self):
        ...
        """
        guess_lr, guess_rl = [
            [str(int(x) + j) for x in self.rhea_rxn_ids["external"]] for j in [1, 2]
        ]
        all_externals = self.rhea_rxn_ids["external"] + guess_lr + guess_rl
        dir_externals = self.rhea_directions.isin(all_externals)
        invalid_externals = list(
            self.rhea_directions[dir_externals == True].stack().values
        )
    ...
    def add_internal_rhea(self):
        ...
        rxn_paths = [
            os.path.join(self.paths["rxn"], rhea_id) + ".rxn"
            for rhea_id in self.rhea_rxn_ids["local"]
        ]
    ...
    def get_rxn_products(self):
        ...
        smiles_df = pd.DataFrame.from_dict(
            flat_smiles, orient="index", columns=["smiles"]
        )
    ...
    def star_to_smiles(self):
        ...
        related_chebis = [
            u.chebi_rels(chebi)
            for chebi in list(self.smiles_stars["ChEBI_ID"])
            if "CHEBI" in chebi
        ]
        flattened_related = ["CHEBI:" + str(i) for s in related_chebis for i in s]
        matched_chebis = u.get_chebi_smiles(
            pd.read_csv(self.paths["chebi_smiles"], sep="\t"), flattened_related
        )
        all_smiles = matched_chebis.drop_duplicates(
            subset="ChEBI_ID", ignore_index=True
        )
        nostars = pd.concat(
